...
document.addEventListener("submit", function (event) {
    const form = event.target.closest(".bookmark-form");
    if (!form) return;
    ...
    const postId = url.split("/")[4]; // URLから投稿IDを取得
    ...
    fetch(url, {
        ...
        .then((data) => {
            if (data.success) {
                const bookmarkSection = document.getElementById(
                    `bookmark-section-${postId}`
                );
                ...
                if (isUnbookmark) {
                    // ブックマークするためのフォームに書き換える
                    const bookmarkUrl = url.replace("/bookmark", "/bookmark"); // URLは同じ
                    newHtml = `
